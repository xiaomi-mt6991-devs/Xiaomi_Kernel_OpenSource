...
/*
 *  mt6899-mt6368.c  --  mt6899 mt6368 ALSA SoC machine driver
 *
 ...
 * mt6899_mt6368_spk_amp_event()
 */
#define EXT_SPK_AMP_W_NAME "Ext_Speaker_Amp"
#define BYPASS_CODEC_PROBE 0
...
static struct snd_soc_card mt6899_mt6368_soc_card;

struct mt6899_compress_info compr_info;

static const char *const mt6899_spk_type_str[] = {MTK_SPK_NOT_SMARTPA_STR,
...
static const char *const
	mt6899_spk_i2s_type_str[] = {MTK_SPK_I2S_0_STR,
				     MTK_SPK_I2S_1_STR,
				     MTK_SPK_I2S_2_STR,
				     MTK_SPK_I2S_3_STR,
				     MTK_SPK_I2S_5_STR,
...
static const struct soc_enum mt6899_spk_type_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6899_spk_type_str),
			    mt6899_spk_type_str),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6899_spk_i2s_type_str),
			    mt6899_spk_i2s_type_str),
};

static int mt6899_spk_type_get(struct snd_kcontrol *kcontrol,
...
static int mt6899_spk_i2s_out_type_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6899_spk_i2s_in_type_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6899_compress_info_set(struct snd_kcontrol *kcontrol,
				    const unsigned int __user *data,
				    unsigned int size)
{
	if (copy_from_user(&compr_info,
			   data,
			   sizeof(struct mt6899_compress_info))) {
...
static int mt6899_compress_info_get(struct snd_kcontrol *kcontrol,
				    unsigned int __user *data, unsigned int size)
{
	struct snd_soc_card *card = &mt6899_mt6368_soc_card;
	...
	if (copy_to_user(data, &compr_info, sizeof(struct mt6899_compress_info))) {
...
static int mt6899_mt6368_spk_amp_event(struct snd_soc_dapm_widget *w,
					...
					int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6899_afe_gpio_request(afe, true, MT6899_GPIO_EXT_HP_AMP, 0);
#endif
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* spk amp off control */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6899_afe_gpio_request(afe, false, MT6899_GPIO_EXT_HP_AMP, 0);
#endif
		break;
	default:
		break;
...
static const struct snd_soc_dapm_widget mt6899_mt6368_widgets[] = {
	SND_SOC_DAPM_SPK(EXT_SPK_AMP_W_NAME, mt6899_mt6368_spk_amp_event),
};

static const struct snd_soc_dapm_route mt6899_mt6368_routes[] = {
...
static const struct snd_soc_dapm_route mt6899_mt6368_routes_dummy[] = {};

static const struct snd_kcontrol_new mt6899_mt6368_controls[] = {
	SOC_DAPM_PIN_SWITCH(EXT_SPK_AMP_W_NAME),
	SOC_ENUM_EXT("MTK_SPK_TYPE_GET", mt6899_spk_type_enum[0],
		     mt6899_spk_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_OUT_TYPE_GET", mt6899_spk_type_enum[1],
		     mt6899_spk_i2s_out_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_IN_TYPE_GET", mt6899_spk_type_enum[1],
		     mt6899_spk_i2s_in_type_get, NULL),
	SND_SOC_BYTES_TLV("MTK_COMPRESS_INFO",
			  sizeof(struct mt6899_compress_info),
			  mt6899_compress_info_get, mt6899_compress_info_set),
};

/*
 * define mtk_spk_i2s_mck node in dts when need mclk,
 * BE i2s need assign snd_soc_ops = mt6899_mt6368_i2s_ops
 */
static int mt6899_mt6368_i2s_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_ops mt6899_mt6368_i2s_ops = {
	.hw_params = mt6899_mt6368_i2s_hw_params,
};

static int mt6899_mt6368_mtkaif_calibration(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!ret)
		dev_info(afe->dev, "%s(), pm_runtime_get_sync error", __func__);

	miso0_need_calib = mt6899_afe_gpio_is_prepared(MT6899_AFE_GPIO_DAT_MISO0_ON);
	miso1_need_calib = mt6899_afe_gpio_is_prepared(MT6899_AFE_GPIO_DAT_MISO1_ON);
	miso2_need_calib = mt6899_afe_gpio_is_prepared(MT6899_AFE_GPIO_DAT_MISO2_ON);

	mt6899_afe_gpio_request(afe, true, MT6899_DAI_ADDA, 1);
	mt6899_afe_gpio_request(afe, true, MT6899_DAI_ADDA, 0);
	mt6899_afe_gpio_request(afe, true, MT6899_DAI_ADDA_CH34, 1);
	mt6899_afe_gpio_request(afe, true, MT6899_DAI_ADDA_CH34, 0);
	...
	for (phase = 0;
		...
		while (test_done_1 == 0 ||
		       test_done_2 == 0 ||
		       test_done_3 == 0) {
			regmap_read(afe_priv->topckgen, CKSYS_AUD_TOP_MON, &monitor);
	...
	mt6899_afe_gpio_request(afe, false, MT6899_DAI_MTKAIF, 1);
	mt6899_afe_gpio_request(afe, false, MT6899_DAI_ADDA, 0);
	mt6899_afe_gpio_request(afe, false, MT6899_DAI_ADDA_CH34, 1);
	mt6899_afe_gpio_request(afe, false, MT6899_DAI_ADDA_CH34, 0);
	...
	if (!ret)
		dev_info(afe->dev, "%s(), pm_runtime_put error", __func__);
...
static int mt6899_mt6368_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
	...
	ops.enable_dc_compensation = mt6899_enable_dc_compensation;
	ops.set_lch_dc_compensation = mt6899_set_lch_dc_compensation;
	ops.set_rch_dc_compensation = mt6899_set_rch_dc_compensation;
	ops.adda_dl_gain_control = mt6899_adda_dl_gain_control;
	mt6368_set_codec_ops(codec_component, &ops);

	if (BYPASS_CODEC_PROBE) {
		dev_info(afe->dev, "%s(), BYPASS_CODEC_PROBE == true, gonna return 0\n", __func__);
		return 0;
	}
	...
	//register_get_power_scene_callback(mt6899_aud_get_power_scenario);

	/* mtkaif calibration */
	mt6899_mt6368_mtkaif_calibration(rtd);
...
static int mt6899_i2s_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				      struct snd_pcm_hw_params *params)
{
...
static const struct snd_pcm_hardware mt6899_mt6368_vow_hardware = {
...
static int mt6899_mt6368_vow_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	snd_soc_set_runtime_hwparams(substream, &mt6899_mt6368_vow_hardware);

	mt6899_afe_gpio_request(afe, true, MT6899_DAI_VOW, 0);

	/* ASoC will call pm_runtime_get, but vow don't need */
	for_each_rtd_components(rtd, i, component) {
		ret = pm_runtime_put_autosuspend(component->dev);
		if (!ret)
			dev_info(afe->dev, "%s(), pm_runtime_put_autosuspend error", __func__);
	}

	return 0;
}

static void mt6899_mt6368_vow_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	mt6899_afe_gpio_request(afe, false, MT6899_DAI_VOW, 0);
...
static const struct snd_soc_ops mt6899_mt6368_vow_ops = {
	.startup = mt6899_mt6368_vow_startup,
	.shutdown = mt6899_mt6368_vow_shutdown,
};
...
SND_SOC_DAILINK_DEFS(hostless_src_aaudio,
	...
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
#if IS_ENABLED(CONFIG_SND_SOC_MTK_BTCVSD) && !defined(SKIP_SB_BTCVSD)
...
static struct snd_soc_dai_link mt6899_mt6368_dai_links[] = {
	/* Front End DAI links */
	{
		.name = "Playback_1",
...
static struct snd_soc_card mt6899_mt6368_soc_card = {
	.name = "mt6899-mt6368",
	.owner = THIS_MODULE,
	.dai_link = mt6899_mt6368_dai_links,
	.num_links = ARRAY_SIZE(mt6899_mt6368_dai_links),

	.controls = mt6899_mt6368_controls,
	.num_controls = ARRAY_SIZE(mt6899_mt6368_controls),
	.dapm_widgets = mt6899_mt6368_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt6899_mt6368_widgets),
	.dapm_routes = mt6899_mt6368_routes,
	.num_dapm_routes = ARRAY_SIZE(mt6899_mt6368_routes),
};

static int mt6899_mt6368_bypass_primary_codec(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt6899_mt6368_soc_card;
	int i;
	struct snd_soc_dai_link *dai_link;
	...
	card->dapm_routes = mt6899_mt6368_routes_dummy;
	card->num_dapm_routes = ARRAY_SIZE(mt6899_mt6368_routes_dummy);
	return 0;
}

static int mt6899_mt6368_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt6899_mt6368_soc_card;
	...
	if(get_smartpa_type() == SMARTPA_FS19XX){
		...
		dev_info(&pdev->dev, "%s(), smartpa is FS19XX\n",__func__);
	}else{
		spk_node = of_get_child_by_name(pdev->dev.of_node,
			"mediatek,speaker-codec");
		dev_info(&pdev->dev, "%s(), smartpa is Not FS19XX\n",__func__);
	}
	if (!spk_node) {
		dev_info(&pdev->dev,
	...
	/* Bypass codec driver */
	if (BYPASS_CODEC_PROBE) {
		dev_info(&pdev->dev, "%s Bypass codec driver\n", __func__);
		mt6899_mt6368_bypass_primary_codec(pdev);
	}
...
static const struct of_device_id mt6899_mt6368_dt_match[] = {
	{.compatible = "mediatek,mt6899-mt6368-sound",},
	{}
};
#endif

static const struct dev_pm_ops mt6899_mt6368_pm_ops = {
	.poweroff = snd_soc_poweroff,
	.restore = snd_soc_resume,
};

static struct platform_driver mt6899_mt6368_driver = {
	.driver = {
		.name = "mt6899-mt6368",
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = mt6899_mt6368_dt_match,
#endif
		.pm = &mt6899_mt6368_pm_ops,
	},
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("mt6899 mt6368 soc card");

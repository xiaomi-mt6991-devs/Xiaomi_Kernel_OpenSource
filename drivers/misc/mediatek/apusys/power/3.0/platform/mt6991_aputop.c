...
static int apu_throttle(struct apu_cooling_device *apu_cdev, unsigned long state)
{
	...
	memset(&aputop, apu_cdev->max_state, sizeof(struct aputop_func_param));
	...
	mt6991_aputop_opp_limit(&aputop, OPP_LIMIT_THERMAL);
	...
	unsigned long state)
{
	...
	if (WARN_ON(state > apu_cdev->max_state || !apu_cdev->throttle || state < APU_COOLING_UNLIMITED_STATE))
		return -EINVAL;

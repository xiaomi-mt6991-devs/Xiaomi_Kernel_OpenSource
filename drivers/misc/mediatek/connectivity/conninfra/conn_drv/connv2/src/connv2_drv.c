...
#define CONN_DUMP_CHIP_INFO_BUF_SIZE 128
	int ret = 0, len;
	char tmp_buf[CONN_DUMP_CHIP_INFO_BUF_SIZE];

	memset(tmp_buf, '\0', CONN_DUMP_CHIP_INFO_BUF_SIZE);
	ret = conninfra_core_get_chip_info(tmp_buf, CONN_DUMP_CHIP_INFO_BUF_SIZE);
	if (ret)
		return ret;

...
bool check_print_msg_info(const struct ipi_msg_t *p_ipi_msg)
{
	...
	if (p_ipi_msg->task_scene == TASK_SCENE_DYNAMIC &&
	    p_ipi_msg->msg_id == AUDIO_DSP_TASK_DLCOPY)
		return false;
...
#if IS_ENABLED(CONFIG_VHOST_ADSP)
...
int send_message_to_dsp(const struct ipi_msg_t *p_ipi_msg)
{
	int send_status = 0;
	uint32_t wait_ms = 0;
	...
	if (dsp_id == AUDIO_OPENDSP_ID_INVALID) {
		DUMP_IPI_MSG("dsp_id error", p_ipi_msg);
		return 0;
	}

	ipi_id = audio_get_audio_ipi_id(p_ipi_msg->task_scene);
	if (ipi_id == 0xFFFFFFFF) {
		DUMP_IPI_MSG("ipi_id error", p_ipi_msg);
		return 0;
	}

	send_status = dsp_send_msg_to_queue(
			      dsp_id,
			      ipi_id,
			      (void *)p_ipi_msg,
	...
	if (send_status != 0) {
		pr_notice("dsp_send_msg_to_queue error %d", send_status);
		DUMP_IPI_MSG("send fail", p_ipi_msg);
	}

...
#endif
{
	...
	if (!hcp_dev->data->get_omc_virt) {
		dev_info(&pdev->dev, "%s: not supported\n", __func__);
		return NULL;
	}
#if SMVR_DECOUPLE
    buffer = hcp_dev->data->get_omc_virt(mode);
#else
	buffer = hcp_dev->data->get_omc_virt();
#endif
	if (!buffer)
		dev_info(&pdev->dev, "%s: omc cq buffer is null\n", __func__);

	return buffer;
}
EXPORT_SYMBOL(mtk_hcp_get_omc_mem_virt);
...
#endif
{
	...
	if (!hcp_dev->data->get_omc_cq_fd) {
		dev_info(&pdev->dev, "%s: not supported\n", __func__);
		return -1;
	}
#if SMVR_DECOUPLE
	fd = hcp_dev->data->get_omc_cq_fd(mode);
#else
	fd = hcp_dev->data->get_omc_cq_fd();
#endif
	if (fd < 0)
		dev_info(&pdev->dev, "%s: omc cq fd is wrong\n", __func__);

	return fd;
}
EXPORT_SYMBOL(mtk_hcp_get_omc_mem_cq_fd);
...
#endif
{
	...
	if (!hcp_dev->data->get_omc_tdr_fd) {
		dev_info(&pdev->dev, "%s: not supported\n", __func__);
		return -1;
	}
#if SMVR_DECOUPLE
	fd = hcp_dev->data->get_omc_tdr_fd(mode);
#else
	fd = hcp_dev->data->get_omc_tdr_fd();
#endif
	if (fd < 0)
		dev_info(&pdev->dev, "%s: omc tdr fd is wrong\n", __func__);

	return fd;
}
EXPORT_SYMBOL(mtk_hcp_get_omc_mem_tdr_fd);
...
#else
void*mtk_hcp_get_me_mem_virt(struct platform_device *pdev)
#endif
{
	struct mtk_hcp *hcp_dev = platform_get_drvdata(pdev);
	void *buffer = NULL;

	if (!hcp_dev->data->get_me_virt) {
		dev_info(&pdev->dev, "%s: not supported\n", __func__);
		return NULL;
	}
#if SMVR_DECOUPLE
    buffer = hcp_dev->data->get_me_virt(mode);
#else
	buffer = hcp_dev->data->get_me_virt();
#endif
	if (!buffer)
		dev_info(&pdev->dev, "%s: me cq buffer is null\n", __func__);

	return buffer;
}
EXPORT_SYMBOL(mtk_hcp_get_me_mem_virt);

#if SMVR_DECOUPLE
int mtk_hcp_get_me_mem_cq_fd(struct platform_device *pdev, unsigned int mode)
#else
int mtk_hcp_get_me_mem_cq_fd(struct platform_device *pdev)
#endif
{
	struct mtk_hcp *hcp_dev = platform_get_drvdata(pdev);
	int fd = -1;

	if (!hcp_dev->data->get_me_cq_fd) {
		dev_info(&pdev->dev, "%s: not supported\n", __func__);
		return -1;
	}
#if SMVR_DECOUPLE
	fd = hcp_dev->data->get_me_cq_fd(mode);
#else
	fd = hcp_dev->data->get_me_cq_fd();
#endif
	if (fd < 0)
		dev_info(&pdev->dev, "%s: me cq fd is wrong\n", __func__);

	return fd;
}
EXPORT_SYMBOL(mtk_hcp_get_me_mem_cq_fd);
...
static const struct of_device_id mtk_hcp_match[] = {
    {.compatible = "mediatek,hcp8", .data = (void *)&isp8_hcp_data},
	{}
};

...
void tscpu_thermal_cal_prepare(struct platform_device *pdev)
{
	...
	if (IS_ERR(nvmem_dev)) {
		tscpu_warn("%s failed to get nvmem_dev\n", __func__);
		return;
	}

	ret = nvmem_device_read(nvmem_dev, ADDRESS_OFFSET_0, sizeof(__u32), &temp0);
	if (ret < 0)
		temp0 = 0xFFFFFFFF;

	ret = nvmem_device_read(nvmem_dev, ADDRESS_OFFSET_1, sizeof(__u32), &temp1);
	if (ret < 0)
		temp1 = 0xFFFFFFFF;

	ret = nvmem_device_read(nvmem_dev, ADDRESS_OFFSET_2, sizeof(__u32), &temp2);
	if (ret < 0)
		temp2 = 0xFFFFFFFF;
...
int tscpu_get_curr_temp(void)
{
	 ...
	 */
	tscpu_curr_cpu_temp = MAX(tscpu_ts_temp[TS_MCU4],
		tscpu_ts_temp[TS_MCU5]);

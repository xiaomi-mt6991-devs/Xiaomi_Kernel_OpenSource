...
static int mtk_vdisp_probe(struct platform_device *pdev)
{
	...
	pm_runtime_get(dev);
	mtk_vdisp_register(&funcs, VDISP_VER1);
...
struct platform_driver mtk_vdisp_driver_v1 = {
	.probe = mtk_vdisp_probe,
	.remove = mtk_vdisp_remove,
	.driver = {
		.name = "mediatek-vdisp-ctrl-v1",
		.owner = THIS_MODULE,
		.of_match_table = mtk_vdisp_driver_v1_dt_match,
	},
};
...
{
	VDISPDBG("+");
	platform_driver_register(&mtk_vdisp_driver_v1);
	VDISPDBG("-");
	return 0;
}

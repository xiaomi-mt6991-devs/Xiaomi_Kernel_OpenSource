...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0x05, 0x40);/* VGH=2xAVDD, VGL=2xAVEE */
	...
	lcm_dcs_write_seq_static(ctx, 0x11, 0x0D);/* VCOM */
	lcm_dcs_write_seq_static(ctx, 0x12, 0x10);/* VCOM */
	...
	lcm_dcs_write_seq_static(ctx, 0x20, 0x00);/* STV */
	...
	lcm_dcs_write_seq_static(ctx, 0xBD, 0x20);/* VEND */
	...
	lcm_dcs_write_seq_static(ctx, 0xC4, 0x24);/* updated */

	lcm_dcs_write_seq_static(ctx, 0xBE, 0x07);
	lcm_dcs_write_seq_static(ctx, 0xBF, 0x07);
	lcm_dcs_write_seq_static(ctx, 0x29, 0xD8);/* UD */
	lcm_dcs_write_seq_static(ctx, 0x2A, 0x2A);

	lcm_dcs_write_seq_static(ctx, 0x5B, 0x43);/* CTRL */
	lcm_dcs_write_seq_static(ctx, 0x5C, 0x00);
	...
	lcm_dcs_write_seq_static(ctx, 0x7A, 0x80);/* MUX */
	...
	lcm_dcs_write_seq_static(ctx, 0x93, 0x06);/* FP,BP */
	...
	lcm_dcs_write_seq_static(ctx, 0x78, 0x00);/* SOURCE EQ */
	...
	lcm_dcs_write_seq_static(ctx, 0x8A, 0x33);/* /Inversion Type// pixel column driving */
	...
	lcm_dcs_write_seq_static(ctx, 0xB3, 0x00);/* RN GAMMA SETTING */
	...
	lcm_dcs_write_seq_static(ctx, 0xEF, 0x00);/* GP GAMMA SETTING */
	...
	lcm_dcs_write_seq_static(ctx, 0xFF, 0x21);/* CMD2 PAGE1 */
	lcm_dcs_write_seq_static(ctx, 0xFB, 0x01);
	...
	lcm_dcs_write_seq_static(ctx, 0x71, 0x00);/* BP GAMMA SETTING */
	...
	lcm_dcs_write_seq_static(ctx, 0xC9, 0x49, 0x02, 0x05, 0x00,
				0x0F, 0x06, 0x67, 0x03,
				0x2E, 0x10, 0xF0);

	lcm_dcs_write_seq_static(ctx, 0xBB, 0x03);/* 0x03:video mode  0x10:command mode */
...
static void change_drm_disp_mode_params(struct drm_display_mode *mode)
{
	...
	if (fake_heigh == 1440 && fake_width == 720)
		mode->clock = 70626;
	else if (fake_heigh == 1680 && fake_width == 720)
		mode->clock = 82002;
	else if (fake_heigh == 1520 && fake_width == 720)
		mode->clock = 74418;
}
...
static void check_is_need_fake_resolution(struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node, "fake-height", &fake_heigh);
	if (ret) {
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "nt35695,fhd,vdo,auo,rt5081,hdp", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-nt35695-fhd-auo-vdo-hdp",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("nt35695 auo VDO LCD Panel Driver");
MODULE_LICENSE("GPL");

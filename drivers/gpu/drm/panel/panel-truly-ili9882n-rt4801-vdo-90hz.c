...
static int lcm_dcs_read(struct lcm *ctx, u8 cmd, void *data, size_t len)
{
	...
	if (ret < 0) {
		dev_info(ctx->dev, "[error]error %d reading dcs seq:(%#x)\n", ret, cmd);
		ctx->error = ret;
	}
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (IS_ERR(ctx->bias_neg)) {
		dev_info(ctx->dev, "[error]%s: cannot get bias_neg %ld\n",
	...
	if (IS_ERR(ctx->bias_pos)) {
		dev_info(ctx->dev, "[error]%s: cannot get bias_pos %ld\n",
			...
			struct drm_connector *connector)
{
	...
	if (!mode) {
		dev_info(connector->dev->dev, "[error]failed to add mode %ux%ux@%u\n",
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (IS_ERR(ctx->reset_gpio)) {
		dev_info(dev, "[error]%s: cannot get reset-gpios %ld\n",
			__func__, PTR_ERR(ctx->reset_gpio));
		return PTR_ERR(ctx->reset_gpio);
	}
	...
	if (IS_ERR(ctx->bias_pos)) {
		dev_info(dev, "[error]%s: cannot get bias-pos 0 %ld\n",
			__func__, PTR_ERR(ctx->bias_pos));
		return PTR_ERR(ctx->bias_pos);
	}
	...
	if (IS_ERR(ctx->bias_neg)) {
		dev_info(dev, "[error]%s: cannot get bias-neg 1 %ld\n",
			__func__, PTR_ERR(ctx->bias_neg));
		return PTR_ERR(ctx->bias_neg);
	}

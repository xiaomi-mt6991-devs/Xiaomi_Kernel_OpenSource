...
static inline void pwrap_dump_ap_register(void)
{
	...
	for (i = 0; i <= 0x123; i++) {
		...
		dev_notice(wrp->dev, "addr:0x%p = 0x%x\n",
...
static inline void pwrap_dump_pmic_register(void)
{
	...
	dev_notice(wrp->dev, "dump PMIC register\n");
	pwrap_read(wrp, wrp->slave->dew_regs[PWRAP_DEW_DIO_EN], &rdata);
	dev_notice(wrp->dev, "[REG]0x%x=0x%x\n",
...
static void pwrap_logging_at_isr(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		dev_notice(wrp->dev, "INT0 flag 0x%x\n",
			pwrap_readl(wrp, PWRAP_INT0_FLG));
		dev_notice(wrp->dev, "INT1 flag 0x%x\n",
			pwrap_readl(wrp, PWRAP_INT1_FLG));
		dev_notice(wrp->dev, "DCXO_CONN_ADR0=0x%x\n",
			pwrap_readl(wrp, PWRAP_DCXO_CONN_ADR0));
		dev_notice(wrp->dev, "DCXO_CONN_ADR1=0x%x\n",
			pwrap_readl(wrp, PWRAP_DCXO_CONN_ADR1));

	if (HAS_CAP(wrp->master->caps, PWRAP_CAP_MONITOR_V1)) {
		dev_notice(wrp->dev, "MONITOR_CTRL_0=0x%x\n",
			pwrap_readl(wrp, PWRAP_MONITOR_CTRL_0));
		dev_notice(wrp->dev, "MONITOR_CTRL_1=0x%x\n",
			pwrap_readl(wrp, PWRAP_MONITOR_CTRL_1));
		dev_notice(wrp->dev, "MONITOR_CTRL_2=0x%x\n",
			pwrap_readl(wrp, PWRAP_MONITOR_CTRL_2));
		dev_notice(wrp->dev, "MONITOR_CTRL_3=0x%x\n",
			pwrap_readl(wrp, PWRAP_MONITOR_CTRL_3));
	} else if (HAS_CAP(wrp->master->caps, PWRAP_CAP_MONITOR_V2)) {
		dev_notice(wrp->dev, "MONITOR_CTRL_0=0x%x\n",
			pwrap_readl(wrp, PWRAP_MONITOR_CTRL));
		dev_notice(wrp->dev, "MONITOR_TARGET_CHANNEL_0=0x%x\n",
		...
		dev_notice(wrp->dev, "MONITOR_TARGET_ADR_0=0x%x\n",
		...
		dev_notice(wrp->dev, "CH_SEQ_0=0x%x\n",
			pwrap_readl(wrp, PWRAP_CHANNEL_SEQUENCE_0));
		dev_notice(wrp->dev, "CH_SEQ_1=0x%x\n",
			pwrap_readl(wrp, PWRAP_CHANNEL_SEQUENCE_1));
		dev_notice(wrp->dev, "CH_SEQ_2=0x%x\n",
			pwrap_readl(wrp, PWRAP_CHANNEL_SEQUENCE_2));
		dev_notice(wrp->dev, "CH_SEQ_3=0x%x\n",
...
static void pwrap_reenable_pmic_logging(void)
{
	...
	if (HAS_CAP(wrp->master->caps, PWRAP_CAP_MONITOR_V2) ||
		...
		pwrap_read(wrp, wrp->slave->dew_regs[PWRAP_DEW_RECORD_CMD3],
			   &rdata);
		dev_notice(wrp->dev, "REC_CMD3:0x%x (The fourth-last cmd addr)\n",
			(rdata & 0x3fff) << 1);
		...
		pwrap_read(wrp, wrp->slave->dew_regs[PWRAP_DEW_RECORD_CMD4],
			   &rdata);
		dev_notice(wrp->dev, "REC_CMD4:0x%x (The fifth-last cmd addr)\n",
			(rdata & 0x3fff) << 1);
		...
		pwrap_read(wrp, wrp->slave->dew_regs[PWRAP_DEW_RECORD_CMD5],
			   &rdata);
		dev_notice(wrp->dev, "REC_CMD5:0x%x (The sixth-last cmd addr)\n",
			(rdata & 0x3fff) << 1);
	...
	if (sub_return != 0)
		dev_notice(wrp->dev, "en spi debug fail, ret=%x\n", sub_return);
	dev_notice(wrp->dev, "enable spi debug ok\n");

	/* Clear Last three command */
	sub_return = pwrap_write(wrp,
		wrp->slave->dew_regs[PWRAP_DEW_RG_SPI_RECORD_CLR], 0x1);
	if (sub_return != 0)
		dev_notice(wrp->dev, "clr rec cmd fail, ret=%x\n", sub_return);
	sub_return = pwrap_write(wrp,
		wrp->slave->dew_regs[PWRAP_DEW_RG_SPI_RECORD_CLR], 0x0);
	if (sub_return != 0)
		dev_notice(wrp->dev, "clr rec cmd fail, ret=%x\n", sub_return);
	dev_notice(wrp->dev, "clear record command ok\n");
...
void pwrap_dump_all_register(void)
{
	...
	/* add tsx/dcxo temperature log support */
	rdata = pwrap_readl(wrp, PWRAP_SPMINF_STA);
	dev_notice(wrp->dev, "tsx dump reg_addr:0x%x = 0%x\n",
...
static void pwrap_swinf_info(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		dev_notice(wrp->dev, "Dump SWINF Info\n");
		dev_notice(wrp->dev, "SWINF_2_ACC=0x%x\n",
			pwrap_readl(wrp, PMIF_SPI_PMIF_SWINF_2_ACC));
		dev_notice(wrp->dev, "SWINF_2_WDATA_31_0=0x%x\n",
			pwrap_readl(wrp, PMIF_SPI_PMIF_SWINF_2_WDATA_31_0));
		dev_notice(wrp->dev, "SWINF_2_RDATA_31_0=0x%x\n",
			pwrap_readl(wrp, PMIF_SPI_PMIF_SWINF_2_RDATA_31_0));
		dev_notice(wrp->dev, "SWINF_2_STA=0x%x\n",
...
static void pwrap_monitor_info(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		...
		for (i = 0; i <= 31; i++) {
			...
			dev_notice(wrp->dev, "MONITOR_RECORD_%d_WRITE=0x%x\n", i,
					   PMIF_GET_MONITOR_REC_0_WRITE(rdata));
			dev_notice(wrp->dev, "MONITOR_RECORD_%d_CHAN=0x%x\n", i,
					   PMIF_GET_MONITOR_REC_0_CHAN(rdata));
			dev_notice(wrp->dev, "MONITOR_RECORD_%d_CMD=0x%x\n", i,
					   PMIF_GET_MONITOR_REC_0_CMD(rdata));
			dev_notice(wrp->dev, "MONITOR_RECORD_%d_ADDR=0x%x\n", i,
			...
			reg_addr = wrp->base +
			   ...
			   (i * 20);
			rdata = readl(reg_addr);
			dev_notice(wrp->dev, "MONITOR_RECORD_%d_WDATA=0x%x\n", i,
...
static int pwrap_reset_spislave(struct pmic_wrapper *wrp)
{
	...
	if (ret) {
		dev_notice(wrp->dev, "%s fail, ret=%d\n", __func__, ret);
		return ret;
	}
...
static int pwrap_init_sidly(struct pmic_wrapper *wrp)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (rdata == PWRAP_DEW_READ_TEST_VAL) {
			dev_notice(wrp->dev, "[Read Test] pass, SIDLY=%x\n", i);
			pass |= 1 << i;
		}
	}

	if (dly[pass] < 0) {
		dev_notice(wrp->dev, "sidly pass range 0x%x not continuous\n",
				pass);
		return -EIO;
	}
...
static int pwrap_init_cipher(struct pmic_wrapper *wrp)
{
	...
	if (ret) {
		dev_notice(wrp->dev, "cipher data ready@AP fail, ret=%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		dev_notice(wrp->dev, "cipher mode idle fail, ret=%d\n", ret);
		return ret;
	}
...
static int pwrap_mt8135_init_soc_specific(struct pmic_wrapper *wrp)
{
	...
	if (pwrap_write(wrp, wrp->slave->dew_regs[PWRAP_DEW_EVENT_OUT_EN],
	    ...
	    pwrap_write(wrp, wrp->slave->dew_regs[PWRAP_DEW_EVENT_SRC_EN],
			0xffff)) {
		dev_notice(wrp->dev, "enable dewrap fail\n");
		return -EFAULT;
	}
...
static irqreturn_t pwrap_interrupt(int irqno, void *dev_id)
{
	...
	if (HAS_CAP(wrp->master->caps, PWRAP_CAP_INT1_EN)) {
		...
		if ((int0_flg & 0x01) == 0x01) {
			...
			dev_notice(wrp->dev, "[PWRAP] CRC Error\n");
			pwrap_logging_at_isr();
			pwrap_reenable_pmic_logging();
			pwrap_dump_ap_register();
			/*
			...
			if (ret != 0)
				dev_notice(wrp->dev, "clr fail, ret=%x\n", ret);
			ret = pwrap_write(wrp,
		...
		dev_notice(wrp->dev, "unexpected interrupt int=0x%x\n", rdata);
...
static const struct pmic_wrapper_type pwrap_mt6739 = {
	.regs = mt6739_regs,
	.type = PWRAP_MT6739,
	.arb_en_all = 0x3fd25,
...
static const struct pmic_wrapper_type pwrap_mt6761 = {
	.regs = mt6761_regs,
	.type = PWRAP_MT6761,
	.arb_en_all = 0x3fd35,
	.int_en_all = 0x7,
...
static const struct pmic_wrapper_type pwrap_mt6768 = {
	.regs = mt6768_regs,
	.type = PWRAP_MT6768,
	.arb_en_all = 0xfb27f,
...
static const struct pmic_wrapper_type pwrap_mt6771 = {
	.regs = mt6771_regs,
	.type = PWRAP_MT6771,
	.arb_en_all = 0x3fa75,
...
static const struct pmic_wrapper_type pwrap_mt6781 = {
	.regs = mt6781_regs,
	.type = PWRAP_MT6781,
	.arb_en_all = 0xfb27f,
	.int_en_all = 0xfffffffe, /* disable WatchDog Timeout for bit 1 */
...
static const struct pmic_wrapper_type pwrap_mt6789 = {
	.regs = mt6789_regs,
	.type = PWRAP_MT6789,
	.arb_en_all = 0x777f,
	.int_en_all = 0x180000,
...
static const struct pmic_wrapper_type pwrap_mt6877 = {
	.regs = mt6877_regs,
	.regs1 = mt6877_regs1,
	.type = PWRAP_MT6877,
	.arb_en_all = 0x777f,
...
static const struct pmic_wrapper_type pwrap_mt6885 = {
	.regs = mt6885_regs,
	.regs1 = mt6885_regs1,
	.type = PWRAP_MT6885,
	.arb_en_all = 0x777f,
...
static const struct pmic_wrapper_type pwrap_mt8168 = {
	.regs = mt8168_regs,
	.type = PWRAP_MT8168,
	.arb_en_all = 0x3fd25,
	.int_en_all = 0x7,
...
static const struct pmic_wrapper_type pwrap_mt8195 = {
	...
	.spi_w = PWRAP_MAN_CMD_SPI_WRITE,
	.wdt_src = PWRAP_WDT_SRC_MASK_ALL,
	.caps = PWRAP_CAP_INT1_EN | PWRAP_CAP_ARB_V3,
	.init_reg_clock = pwrap_common_init_reg_clock,
	.init_soc_specific = NULL,
};
...
{
	dev_notice(wrp->dev, "%s set value(W1C): %d\n", __func__, offset);

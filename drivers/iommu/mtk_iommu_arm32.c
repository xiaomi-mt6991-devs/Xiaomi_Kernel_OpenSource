...
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_DBG)
		pr_info("%s base:0x%x fault type=0x%x iova=0x%llx pa=0x%llx layer=%d %s, pgtable=0x%x, tfrp_pa=0x%x, 0x114=0x%x\n",
				     ...
				     unsigned int domid)
{
	...
	pr_info("%s, create table data:(%d,%d)", __func__,
		data->plat_data->iommu_type, data->plat_data->iommu_id);
...
#ifndef CONFIG_ARM64
	if (!IS_ENABLED(CONFIG_ARM64) && type != IOMMU_DOMAIN_UNMANAGED) {
		pr_info("[%s] domain alloc returns the dom", __func__);
		return NULL;
	}

#endif
	if (IS_ENABLED(CONFIG_ARM64) && type != IOMMU_DOMAIN_DMA) {
		pr_err("[%s] domain type is wrong", __func__);
		return NULL;
	}
				    ...
				    struct of_phandle_args *args)
{
	...
	if (!fwspec) {
		...
		pr_err("[%s][name: %s ] dev`s ops doesn`t use mtk_iommu_ops\n", __func__, dev_name(dev));
		return -EINVAL;
	}
...
static const struct iommu_ops mtk_iommu_ops = {
	...
	//.put_resv_regions = iommu_put_resv_regions,
#ifndef CONFIG_ARM64
